...
class STANDARD_INFORMATION(obj.CType):
    ...
    def l2tcsv(self, path, record_num, size, offset, MACB):

        ##TO DO: Fix TZ
        timezone = 'UTC'

        if MACB == 'M...':
            unixdatetime = self.ModifiedTime.v() #M...
            MACB_interp = 'Last Changed'
        elif MACB == '.A..':
            unixdatetime = self.FileAccessedTime.v() #.A..
            MACB_interp = 'Last Accessed'
        elif MACB == '..C.':
            unixdatetime = self.MFTAlteredTime.v() #..C.
            MACB_interp = 'Last Modified'
        elif MACB == '...B':
            unixdatetime = self.CreationTime.v() #...B
            MACB_interp = 'Created'
        ...
        return "{0},{1},{2},{8},MEMORY,MEM-MFT,{9},-,-,-,Standard Information<|>Record Number: {3}<|>Size: {4}<|>Offset: {5},-,{6},,-,-,{7}".format(  
            year,
            time,
            timezone,
            record_num,
...
class FILE_NAME(STANDARD_INFORMATION):
    ...
    def l2tcsv(self, path, record_num, size, offset, MACB):
        ...
        ##TO DO: Get TZ working
        timezone = 'UTC'
            
        return "{0},{1},{2},{8},MEMORY,MEM-MFT,{9},-,-,-,File Name<|>Record Number: {3}<|>Size: {4}<|>Offset: {5},-,{6},,-,-,{7}".format(  
            year,
            time,
            timezone,
            record_num,
...
class MFTParser(common.AbstractWindowsCommand):
    ...
    def render_l2tcsv(self, outfd, data):
        ...
        # We'll make an effort to get the filename from $FN for $SI
        # If there is only one $SI with no $FN we dump whatever information it has
        outfd.write("date,time,timezone,MACB,source,sourcetype,type,user,host,short,desc,version,filename,inode,notes,format,extra\n") 
        for offset, mft_entry, attributes in data:
            si = None
            full = ""
            for a, i in attributes:
                if a.startswith("STANDARD_INFORMATION"):
                    if full != "":
                        # if we are here, we've hit one $FN attribute for this entry already and have the full name
                        # so we can dump this $SI
                        outfd.write("{0}\n".format(i.l2tcsv(full, mft_entry.RecordNumber, int(mft_entry.EntryUsedSize), offset, 'M...')))
                        outfd.write("{0}\n".format(i.l2tcsv(full, mft_entry.RecordNumber, int(mft_entry.EntryUsedSize), offset, '.A..')))
                        outfd.write("{0}\n".format(i.l2tcsv(full, mft_entry.RecordNumber, int(mft_entry.EntryUsedSize), offset, '..C.')))
                        outfd.write("{0}\n".format(i.l2tcsv(full, mft_entry.RecordNumber, int(mft_entry.EntryUsedSize), offset, '...B')))
                    elif si != None:
                        # if we are here then we have more than one $SI attribute for this entry
                        # since we don't want to lose its info, we'll just dump it for now
                        # we won't have full path, but we'll have a filename most likely
                    ...
                    if hasattr(i, "ParentDirectory"):
                        ...
                        outfd.write("{0}\n".format(i.l2tcsv(full, mft_entry.RecordNumber, int(mft_entry.EntryUsedSize), offset, '...B')))
                        if si != None:
                            outfd.write("{0}\n".format(si.l2tcsv(full, mft_entry.RecordNumber, int(mft_entry.EntryUsedSize), offset, 'M...')))
                            outfd.write("{0}\n".format(si.l2tcsv(full, mft_entry.RecordNumber, int(mft_entry.EntryUsedSize), offset, '.A..')))
                            outfd.write("{0}\n".format(si.l2tcsv(full, mft_entry.RecordNumber, int(mft_entry.EntryUsedSize), offset, '..C.')))
                            outfd.write("{0}\n".format(si.l2tcsv(full, mft_entry.RecordNumber, int(mft_entry.EntryUsedSize), offset, '...B')))
                            si = None
            if si != None:
                # here we have a lone $SI in an MFT entry with no valid $FN.  This is most likely a non-base entry
                outfd.write("{0}\n".format(si.l2tcsv(full, mft_entry.RecordNumber, int(mft_entry.EntryUsedSize), offset, 'M...')))
